...
class ServerStatusActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val drawerLayout: DrawerLayout =
            binding.serverStatusDrawer // container for the drawer and main content
        ...
        binding.statusTableRow.visibility = View.GONE
        ...
        binding.addToMyServersButton.visibility = View.GONE
        binding.serverStatusProgressBar.visibility = View.GONE

        // show progress bar
        binding.serverStatusProgressBar.visibility = View.VISIBLE

        if (isConnected()) { // check if the user is connected to internet
            when {
                intent.getStringExtra("edition") == "Java" -> {
                    val url = "https://api.mcsrvstat.us/2/${intent.getStringExtra("ip")}" // use the Java edition version of api
                    ...
                    val jsonObjectRequest = JsonObjectRequest(Request.Method.GET, url, null, // create request for json
                        { response -> // show results based on json results
                            ...
                            if (server.online) {
                                ...
                                binding.ipTableRow.visibility = View.VISIBLE
                                binding.statusTableRow.visibility = View.VISIBLE
                                binding.versionTableRow.visibility = View.VISIBLE
                                binding.motdTableRow.visibility = View.VISIBLE
                                binding.addToMyServersButton.visibility = View.VISIBLE

                                // hide error field and progress bar
                                binding.serverStatusErrorTableRow.visibility = View.GONE
                                binding.serverStatusProgressBar.visibility = View.GONE
                                ...
                                // show add to my servers button so they can check the status later
                                binding.addToMyServersButton.visibility = View.VISIBLE
                    ...
                    val requestQueue: RequestQueue = Volley.newRequestQueue(this) // create request queue
                    requestQueue.add(jsonObjectRequest) // send request to queue
                }

                intent.getStringExtra("edition") == "Bedrock" -> {
                    val url = "https://api.mcsrvstat.us/bedrock/2/${intent.getStringExtra("ip")}" // use the Bedrock edition version of api
        ...
        else { // if they are not, do not proceed
            // hide all fields
            binding.ipTableRow.visibility = View.GONE
            binding.versionTableRow.visibility = View.GONE
            binding.motdTableRow.visibility = View.GONE
            ...
            // show error field
            binding.serverStatusErrorTableRow.visibility = View.VISIBLE
            binding.serverStatusError.text = "An internet connection is required to use this app. Please check your connection and try again"
            Toast.makeText(this, "Internet is required for this app", Toast.LENGTH_LONG).show()
        }
    ...
    // thanks to geeksForGeeks for explaining how to check if there is a network connection (https://www.geeksforgeeks.org/how-to-check-internet-connection-in-kotlin/)
    /**
     * Determines if the user is on a network with internet
     */
    private fun isConnected(): Boolean {
        val cm = getSystemService(Context.CONNECTIVITY_SERVICE) as ConnectivityManager // use activity with connectivityManager

        val network = cm.activeNetwork ?: return false // get a network object of the current data network (could be no network)
        val activeNetwork = cm.getNetworkCapabilities(network) ?: return false // get the capabilities of the network (could be none)

        return when {
            activeNetwork.hasTransport(NetworkCapabilities.TRANSPORT_WIFI) -> true // check if the internet type is wifi

            activeNetwork.hasTransport(NetworkCapabilities.TRANSPORT_CELLULAR) -> true // check if the internet type is cellular

            else -> false // if there are no internet types or an error, return false
        }
    }
}